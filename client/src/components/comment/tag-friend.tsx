...
const TagFriend: React.FC<TagFriendProps> = ({
  debouncedSearch,
  handleTaggedFriend,
  classname,
}: TagFriendProps) => {
  ...
  const {
    ...
    queryFn: ({ pageParam }) =>
      getAcceptedFriends({ pageParam, debouncedSearch: debouncedSearch }),
    initialPageParam: 1,
  ...
  return (
    <>
      {data.pages[0].data.length > 0 && (
        <div
          className={cn(
            "absolute p-2 space-y-2 border rounded-md bg-white shadow-lg z-20",
            classname
          )}
        >
